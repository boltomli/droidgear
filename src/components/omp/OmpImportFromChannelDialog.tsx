...
interface OmpImportFromChannelDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onImported: (result: OmpImportResult) => void
...
/** Infer OMP API type from platform string */
function inferOmpApiType(
...
export function OmpImportFromChannelDialog({
  open,
  onOpenChange,
  onImported,
  existingProviderIds,
}: OmpImportFromChannelDialogProps) {
    ...
    if (isApiKeyAuthChannel(selectedChannel.type)) {
      ...
      try {
        ...
        if (result.status !== 'ok' || !result.data) {
          setResolveError(
            t('omp.provider.importDialog.noApiKey', {
        ...
        setResolvedBaseUrl(
          normalizeBaseUrlForOmp(selectedChannel.baseUrl, null)
        )
        setResolvedPlatform(null)
  ...
  const handleTokenSelect = async (token: ChannelToken) => {
    ...
    setResolvedBaseUrl(normalizeBaseUrlForOmp(rawBaseUrl, token.platform))
  ...
  const handleImport = () => {
    ...
    const api = inferOmpApiType(resolvedPlatform, firstModelId)
    const ompModels: OmpModel[] = Array.from(selectedModelIds).map(id =>
      enrichOmpModelFromRegistry({
        id,
        name: null,
        api: null,
        reasoning: false,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>{t('omp.provider.importDialog.title')}</DialogTitle>
          <DialogDescription>
            {step === 'channel' && t('omp.provider.importDialog.selectChannel')}
            {step === 'token' && t('omp.provider.importDialog.selectToken')}
            {step === 'configure' &&
              t('omp.provider.importDialog.configureProvider')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="space-y-4 py-2">
              {enabledChannels.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">
                  {t('omp.provider.importDialog.noChannels')}
                </p>
              ) : (
                <>
                  <div className="space-y-2">
                    <Label>{t('omp.provider.importDialog.channel')}</Label>
                    <Select
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            'omp.provider.importDialog.channelPlaceholder'
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
              ...
              {tokenFetchState?.isLoading ? (
                <div className="flex items-center justify-center py-6 gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {t('omp.provider.importDialog.loadingTokens')}
                  </span>
                </div>
                ...
                <p className="text-sm text-muted-foreground text-center py-4">
                  {t('omp.provider.importDialog.noTokens')}
                </p>
              ) : (
          ...
          {step === 'configure' && (
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <Label>{t('omp.provider.importDialog.providerId')} *</Label>
                <Input
                  value={providerId}
                ...
                {providerIdConflict && (
                  <p className="text-xs text-destructive">
                    {t('omp.provider.importDialog.providerIdConflict')}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label>{t('omp.provider.importDialog.models')}</Label>
                  {models.length > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {t('omp.provider.importDialog.selectAll')}
                      </Label>
                    </div>
                  )}
                </div>
                {isFetchingModels ? (
                  ...
                  <p className="text-xs text-muted-foreground py-2">
                    {t('omp.provider.importDialog.noModelsHint')}
                  </p>
                ) : (
                ...
                <p className="text-xs text-muted-foreground">
                  {t('omp.provider.importDialog.modelsHint', {
        ...
        <DialogFooter className="gap-2">
          ...
          {step === 'configure' && (
            <Button onClick={handleImport} disabled={!canImport}>
              {t('omp.provider.importDialog.import')}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
