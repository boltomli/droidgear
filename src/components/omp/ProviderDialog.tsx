...
import { enrichOmpModelFromRegistry } from '@/lib/omp-model-metadata'
import type {
  OmpProfile,
  OmpProviderConfig,
  OmpModel,
...
import { OmpModelRegistryDialog } from './OmpModelRegistryDialog'
...
function ProviderForm({
  ...
  const updateProvider = useOmpStore(state => state.updateProvider)
  ...
  const [models, setModels] = useState<OmpModel[]>(existingConfig?.models ?? [])
  ...
  const handleImportFromChannel = useCallback((result: OmpImportResult) => {
    setModels(prev => [...prev, ...result.models])
  }, [])

  const handleAddFromRegistry = (model: OmpModel) => {
  ...
  const handleFillFromRegistry = (index: number) => {
    ...
    if (!model) return
    const enriched = enrichOmpModelFromRegistry(model)
    if (enriched === model) {
      toast.warning(t('omp.provider.registry.notFound'))
      return
    }
  ...
  const handleModelCompatChange = (index: number, value: string) => {
    ...
    if (result.valid && value.trim()) {
      try {
        const parsed = JSON.parse(value) as OmpCompatConfig
        const updated = [...models]
        const model = updated[index]
        if (!model) return
  ...
  const handleSave = () => {
    ...
    if (hasValidationErrors()) {
      toast.error(t('omp.provider.validationError'))
      return
    }
    ...
    if (headersStr.trim()) {
      try {
        ...
        toast.error(t('omp.provider.headersInvalid'))
        return
      }
    }

    let compat: OmpCompatConfig | null = null
    if (compatStr.trim()) {
      try {
        compat = JSON.parse(compatStr)
      } catch {
        toast.error(t('omp.provider.compatInvalid'))
        return
      }
    }
    ...
    if (modelOverridesStr.trim()) {
      try {
        ...
        toast.error(t('omp.provider.modelOverridesInvalid'))
        return
      }
    }
    ...
    const config: OmpProviderConfig = {
      ...
      headers: headers as OmpProviderConfig['headers'],
      authHeader: authHeader || null,
      models: validModels,
      modelOverrides: modelOverrides as OmpProviderConfig['modelOverrides'],
      compat,
    }
  ...
  return (
    <>
      ...
      >
        ...
        {/* Base URL */}
        <div className="space-y-2">
          <Label>{t('omp.provider.baseUrl')}</Label>
          <Input
            value={baseUrl}
        ...
        {/* API Type */}
        <div className="space-y-2">
          <Label>{t('omp.provider.apiType')}</Label>
        ...
        {/* API Key */}
        <div className="space-y-2">
          <Label>{t('omp.provider.apiKey')}</Label>
          <SecretInput
            value={apiKey}
          ...
          <p className="text-xs text-muted-foreground">
            {t('omp.provider.apiKeyHint')}
          </p>
        </div>

        {/* Headers JSON */}
        <JsonField
          label={t('omp.provider.headers')}
          value={headersStr}
          onChange={val => {
            setHeadersStr(val)
            validateHeaders(val)
        ...
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="authHeader" className="cursor-pointer">
            {t('omp.provider.authHeader')}
          </Label>
        </div>

        {/* Compat JSON */}
        <JsonField
          label={t('omp.provider.compat')}
          value={compatStr}
          onChange={val => {
            setCompatStr(val)
            validateCompat(val)
        ...
        <JsonField
          label={t('omp.provider.modelOverrides')}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              >
                ...
                {t('omp.provider.addFromRegistry')}
              </Button>
              <Button
                type="button"
                variant="outline"
                ...
                <div key={index} className="p-3 border rounded-lg space-y-2">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          title={t('omp.provider.fillFromRegistry')}
                        >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {t('omp.provider.reasoning')}
                      </Label>
                    </div>
                    {/* Input types */}
                    <div className="flex items-center gap-2">
                      <Label className="text-xs">
                        {t('omp.provider.inputTypes')}:
                      </Label>
                      <div className="flex items-center gap-1">
                        <Checkbox
                          id={`input-text-${index}`}
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs">{t('omp.provider.cost')}</Label>
                    <div className="grid grid-cols-4 gap-2">
                      <div>
                        <Label className="text-[10px] text-muted-foreground">
                          {t('omp.provider.costInput')}
                        </Label>
                        <Input
                          type="number"
                          step="0.000001"
                  ...
                  {/* Model Compat JSON */}
                  <JsonField
                    label={t('omp.provider.modelCompat')}
                    value={
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            {isEditing ? t('omp.provider.edit') : t('omp.provider.add')}
          </DialogTitle>
          <DialogDescription>
            {t('omp.provider.dialogDescription')}
          </DialogDescription>
        </DialogHeader>
        {open && (
          <ProviderForm
