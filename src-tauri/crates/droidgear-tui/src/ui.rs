...
fn draw_main(frame: &mut Frame, app: &app::App, area: Rect) {
    match app.screen {
        ...
        app::Screen::OmpProvider => draw_omp_provider(frame, app, area),
        app::Screen::OmpModel => draw_omp_model(frame, app, area),
        ...
        app::Screen::CodexAuth => draw_codex_auth(frame, app, area),
    }
}
...
fn draw_omp_profile(frame: &mut Frame, app: &app::App, area: Rect) {
    ...
    let fields_count = 2usize;
    let in_fields = app.omp_detail_field_index < fields_count;
    let in_providers = !in_fields;
    ...
    let mut field_items: Vec<ListItem> = Vec::new();
    for (i, (label, value)) in fields.into_iter().enumerate() {
        let selected = in_fields && i == app.omp_detail_field_index;
        let line = if selected {
            Line::from(format!("{label:>14}: {value}"))
        } else {
            field_line(label, &value, 14)
    ...
    let field_selected = in_fields.then_some(app.omp_detail_field_index);
    let field_list = List::new(field_items)
    ...
    let provider_highlight = if in_providers && !provider_ids.is_empty() {
        Some(app.omp_detail_field_index - fields_count)
    } else {
        None
    };
    let provider_list = List::new(provider_items)
...
fn draw_omp_provider(frame: &mut Frame, app: &app::App, area: Rect) {
    let t = theme();
    let Some(profile) = app.omp_detail.as_ref() else {
        let p = Paragraph::new(vec![Line::from(Span::styled(
            "Failed to load profile",
    ...
    let mut field_items: Vec<ListItem> = Vec::new();
    for (i, (label, value)) in provider_fields.into_iter().enumerate() {
        let selected = i == app.omp_provider_field_index;
        let line = if selected {
            Line::from(format!("{label:>14}: {value}"))
        } else {
            let custom_style = match label {
    ...
    let fields_list = List::new(field_items)
        .block(block(format!("OMP Provider: {provider_id}")))
        .highlight_style(t.selected_row_style());
    render_list(
        frame,
        fields_list,
        chunks[0],
        Some(app.omp_provider_field_index),
    );
    ...
    let models_selected = (!config.models.is_empty()).then_some(app.omp_model_index);
    let models_list = List::new(model_items)
        .block(block("Models"))
        .highlight_style(t.selected_row_style());
    render_list(frame, models_list, chunks[1], models_selected);
...
fn draw_omp_model(frame: &mut Frame, app: &app::App, area: Rect) {
    let t = theme();
    let Some(profile) = app.omp_detail.as_ref() else {
        let p = Paragraph::new(vec![Line::from(Span::styled(
            "Failed to load profile",
    ...
    let Some(model) = provider.models.get(app.omp_model_index) else {
        let p = Paragraph::new(vec![Line::from(Span::styled(
            "Model not found",
            t.error_style(),
        ))])
    ...
    let mut items: Vec<ListItem> = Vec::new();
    for (i, (label, value)) in fields.into_iter().enumerate() {
        let selected = i == app.omp_model_field_index;
        let line = if selected {
            Line::from(format!("{label:>14}: {value}"))
        } else {
            field_line(label, &value, 14)
    ...
    let list = List::new(items)
        .block(block(format!("OMP Model: {}", model.id)))
        .highlight_style(t.selected_row_style());
    render_list(frame, list, chunks[0], Some(app.omp_model_field_index));
...
fn draw_codex_auth(frame: &mut Frame, app: &app::App, area: Rect) {
    let t = theme();
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Min(0), Constraint::Length(2)].as_ref())
    ...
    let mut lines: Vec<Line> = Vec::new();
    for (i, profile) in app.codex_auth_profiles.iter().enumerate() {
        let selected = i == app.codex_auth_index;
        let style = if selected {
            t.selected_style()
        } else {
            Style::default()
        };
        let active_tag = if app.codex_auth_active.as_deref() == Some(&profile.name) {
            " [active]"
        } else {
            ""
        };
        let official_tag = if profile.is_official {
            " [official]"
        } else {
            ""
        };
        lines.push(Line::from(vec![
            Span::styled(
                format!("{}{}{}", profile.label, official_tag, active_tag),
                style,
            ),
            Span::styled(
                format!("  ({})", profile.name),
