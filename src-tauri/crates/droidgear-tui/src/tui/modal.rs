...
pub(super) fn run_select_action(
    ...
    match action {
        ...
        app::SelectAction::OmpSetProviderApi {
            profile_id,
            provider_id,
        } => {
            let Some(selected) = selected else {
            ...
            let mut profile =
                droidgear_core::omp::get_omp_profile_for_home(&app.home_dir, &profile_id)
                    .map_err(anyhow::Error::msg)?;
            let Some(provider) = profile.providers.get_mut(&provider_id) else {
                return Err(anyhow::Error::msg("Provider not found"));
            };
        ...
        app::SelectAction::OmpImportFromChannel {
            profile_id,
            provider_id,
        } => {
            let Some(selected) = selected else {
            ...
            app.omp_import_pending_channel_id = Some(channel.id.clone());
            app.omp_import_pending_base_url = Some(channel.base_url.clone());
            ...
            if is_token_based {
                    ...
                    if !password.is_empty() {
                        match droidgear_core::channel::fetch_channel_tokens_blocking(
                            ...
                            Ok(tokens) => {
                                ...
                                // Store tokens for platform lookup
                                app.omp_import_pending_tokens = Some(tokens);
                                app.modal = Some(app::Modal::Select {
                                    title: "Select token".to_string(),
                                    options,
                                    index: 0,
                                    action: app::SelectAction::OmpImportSetToken {
                                        profile_id,
                                        provider_id,
                                    },
                                });
                        ...
                        app.modal = Some(app::Modal::Input {
                            ...
                            cursor: usize::MAX,
                            is_secret: true,
                            action: app::InputAction::OmpImportSetApiKey {
                                profile_id,
                                provider_id,
                            },
                        });
                ...
                if let Some(api_key) = api_key {
                    ...
                    match droidgear_core::channel::fetch_models_by_api_key_blocking(
                        ...
                        Ok(models) => {
                            // Store models and show MultiSelect
                            app.omp_import_pending_models = Some(models.clone());
                            let selected_bools: Vec<bool> = vec![true; models.len()];
                            app.omp_import_pending_selected = Some(selected_bools.clone());
                            app.omp_import_pending_api_key = Some(api_key);
                            ...
                            app.modal = Some(app::Modal::MultiSelect {
                                ...
                                selected: selected_bools,
                                index: 0,
                                action: app::SelectAction::OmpImportToggleModel {
                                    profile_id,
                                    provider_id,
                                },
                            });
        ...
        app::SelectAction::OmpImportSetToken {
            profile_id,
            provider_id,
        } => {
            let Some(selected) = selected else {
            ...
            let base_url = app.omp_import_pending_base_url.clone().unwrap_or_default();

            // Look up the selected token to get its platform
            let platform = app.omp_import_pending_tokens.as_ref().and_then(|tokens| {
                tokens.iter().find_map(|t| {
                    let display = format!("{} ({})", t.name, t.key);
                    if display == selected {
                        t.platform.clone()
            ...
            .to_string();
            app.omp_import_pending_api_type = Some(api_type);
        ...
        app::SelectAction::OmpImportToggleModel {
            profile_id,
            provider_id,
        } => {
            // User confirmed the model selection - import selected models
            let models = app.omp_import_pending_models.take().unwrap_or_default();
            let selected = app.omp_import_pending_selected.take().unwrap_or_default();
            let api_key = app.omp_import_pending_api_key.take().unwrap_or_default();
            let base_url = app.omp_import_pending_base_url.take().unwrap_or_default();

            let mut profile =
                droidgear_core::omp::get_omp_profile_for_home(&app.home_dir, &profile_id)
                    .map_err(anyhow::Error::msg)?;

            let omp_models: Vec<droidgear_core::omp::OmpModel> = models
                .into_iter()
                .enumerate()
                .filter(|(i, _)| i < &selected.len() && selected[*i])
                .map(|(_, m)| {
                    let mut model = droidgear_core::omp::OmpModel {
                        id: m.id,
                        name: m.name,
                        api: None,
                        reasoning: false,
                    ...
                    droidgear_core::omp::enrich_omp_model_from_registry(&mut model);
                    model
                })
                .collect();
            ...
            if let Some(provider) = profile.providers.get_mut(&provider_id) {
                ...
                provider.api_key = Some(api_key);
                provider.api = app
                    .omp_import_pending_api_type
                    .take()
                    .or_else(|| Some("openai-completions".to_string()));
                provider.models = omp_models;
            }

            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile.clone())
                .map_err(anyhow::Error::msg)?;
            app.omp_detail = Some(profile);
            app.omp_import_pending_provider_id = None;
            app.set_toast("Imported from channel", false);
            Ok(())
        }
        app::SelectAction::OmpAddProviderFromChannel {
            profile_id,
            provider_id,
        } => {
            let Some(selected) = selected else {
...
pub(super) fn run_confirm_action(
    ...
    match action {
        ...
        app::ConfirmAction::OmpDeleteProvider {
            profile_id,
            provider_id,
        } => {
            let mut profile =
        ...
        app::ConfirmAction::OmpDeleteModel {
            profile_id,
            provider_id,
            model_index,
        } => {
...
pub(super) fn run_input_action(
    ...
    match action {
        ...
        app::InputAction::OmpImportSetApiKey {
            profile_id,
            provider_id,
        } => {
            if trimmed.is_empty() {
            ...
            let base_url = app.omp_import_pending_base_url.take();
            let _channel_id = app.omp_import_pending_channel_id.take();
            let Some(base_url) = base_url else {
                return Err(anyhow::anyhow!("No pending channel import"));
            };
            ...
            // Store models and show MultiSelect
            app.omp_import_pending_models = Some(models.clone());
            app.omp_import_pending_api_key = Some(trimmed.to_string());
            let selected_bools: Vec<bool> = vec![true; models.len()];
            app.omp_import_pending_selected = Some(selected_bools.clone());
        ...
        app::InputAction::OmpSetProfileName { id } => {
            if trimmed.is_empty() {
                return Err(anyhow::Error::msg("Profile name is required"));
            }
            let mut profile = droidgear_core::omp::get_omp_profile_for_home(&app.home_dir, &id)
                .map_err(anyhow::Error::msg)?;
            profile.name = trimmed.to_string();
            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile)
                .map_err(anyhow::Error::msg)?;
        ...
        app::InputAction::OmpSetProfileDescription { id } => {
            let mut profile = droidgear_core::omp::get_omp_profile_for_home(&app.home_dir, &id)
                .map_err(anyhow::Error::msg)?;
            profile.description = (!trimmed.is_empty()).then(|| trimmed.to_string());
            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile)
        ...
        app::InputAction::OmpAddProvider { profile_id } => {
            if trimmed.is_empty() {
                return Err(anyhow::Error::msg("Provider id is required"));
            }
            let mut profile =
            ...
            profile.providers.insert(
                trimmed.to_string(),
                droidgear_core::omp::OmpProviderConfig::default(),
            );
            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile)
                .map_err(anyhow::Error::msg)?;
        ...
        app::InputAction::OmpAddProviderFromChannel { profile_id } => {
            if trimmed.is_empty() {
                return Err(anyhow::Error::msg("Provider id is required"));
            }
            // Create the empty provider
            ...
            app.modal = Some(app::Modal::Select {
                ...
                options: enabled_channels,
                index: 0,
                action: app::SelectAction::OmpAddProviderFromChannel {
                    profile_id,
                    provider_id,
                },
            });
        ...
        app::InputAction::OmpSetProviderBaseUrl {
            profile_id,
            provider_id,
        } => {
            let mut profile =
        ...
        app::InputAction::OmpSetProviderApiKey {
            profile_id,
            provider_id,
        } => {
            let mut profile =
        ...
        app::InputAction::OmpAddModel {
            ...
            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile)
                .map_err(anyhow::Error::msg)?;
            app.omp_model_index = new_index;
            app.omp_model_field_index = 0;
            app.screen = app::Screen::OmpModel;
            refresh_omp_detail(app);
            Ok(())
        }
        app::InputAction::OmpSetModelId {
            profile_id,
            provider_id,
            model_index,
        } => {
            ...
            model.id = trimmed.to_string();
            droidgear_core::omp::enrich_omp_model_from_registry(model);
            droidgear_core::omp::save_omp_profile_for_home(&app.home_dir, profile)
                .map_err(anyhow::Error::msg)?;
            app.set_toast("Saved", false);
            Ok(())
        }
        app::InputAction::OmpSetModelName {
            profile_id,
            provider_id,
            model_index,
        } => {
        ...
        app::InputAction::OmpSetModelContextWindow {
            profile_id,
            provider_id,
            model_index,
        } => {
        ...
        app::InputAction::OmpSetModelMaxTokens {
            profile_id,
            provider_id,
            model_index,
        } => {
        ...
        app::InputAction::OmpSetModelCost {
            profile_id,
            provider_id,
            model_index,
        } => {
            ...
            model.cost = Some(droidgear_core::omp::OmpModelCost {
                input,
                output,
                cache_read,
                cache_write,
